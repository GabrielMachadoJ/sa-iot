...
const TotalSpent: FC = () => {
  ...
  const [dataTemperatura, setDataTemperatura] = useState<any>([]);
  ...
  const getData = async () => {
    ...
    response.data.result.forEach((element: any) => {
      ...
      if (element.variable === "temperatura_agua") {
        ...
        setDataTemperatura((prevDataAmonia: any) => [
          ...prevDataAmonia,
          element.value,
        ]);
      }
    ...
    series: [
      {
        ...
        data: dataTemperatura
          ...
          .slice(-10), // Converta os valores para um array [index, value]
      },
    ],
  };
